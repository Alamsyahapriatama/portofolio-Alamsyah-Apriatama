      ...
      <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 gap-6 h-full">
        ...
        >
          ...
          <div class="relative mt-[125px]" v-if="blog">
            <h3
              class="text-center text-white text-3xl md:text-5xl font-semibold"
            >
              {{ blog.title }}
            </h3>
            <h3 class="text-center text-white text-xl md:text-2xl mt-2">
...
const fetchBlog = async (id) => {
  try {
    const response = await fetch(
      `https://cms-les.naditechno.id/api/blog/${id}`
    );
    if (!response.ok) {
